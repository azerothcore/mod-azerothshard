...
public:
    ...
    void OnStartup() override
    {
        ...
        if (!result)
        {
            LOG_INFO("server.loading", ">> Loaded 0 season. DB table `season` is empty.");
        ...
        {
            LOG_INFO("server.loading", ">> No correspondent season found. Check DB table `season`.\n");
            LOG_INFO("server.loading", " ");
        ...
        LOG_INFO("server.loading", ">> Season script for PVP loaded, actual item level: %u\n", sASeasonMgr->GetItemLevel());
        LOG_INFO("server.loading", " ");
    }
};
