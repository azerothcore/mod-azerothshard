...
bool GuildHouseObject::RemoveGuildHouseAdd(uint32 id)
{
    for (uint32 i = 1; i < NPC_MAX; i++)
    {
        ...
        {
            if (GameObjectData const* data = sObjectMgr->GetGameObjectData(*itr2))
            {
...
void GuildHouse::AddGuildHouse_Add(uint32 NewAdd)
{
    ...
    QueryResult result = CharacterDatabase.Query("UPDATE `gh_guildadd` SET `GuildHouse_Add`={} WHERE `guildId`={}", GuildHouse_Add, GuildId);
