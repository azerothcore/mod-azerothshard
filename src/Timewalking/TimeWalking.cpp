...
public:
    ...
    void OnStartup() override
    {
        ...
        {
            LOG_ERROR("Module", ">> Loaded 0 raids for TimeWalking. DB table `timewalking` is empty.\n");
        ...
        do
        {
            ...
            raidList[timeWalking_Field[0].Get<uint32>()] = TwRaid(timeWalking_Field[0].Get<uint32>(), timeWalking_Field[1].Get<std::string>(), timeWalking_Field[2].Get<uint32>(), timeWalking_Field[3].Get<uint32>(), timeWalking_Field[4].Get<uint32>(), timeWalking_Field[5].Get<uint32>(), timeWalking_Field[6].Get<uint32>());
        ...
        {
            LOG_ERROR("Module", ">> Loaded 0 levels for TimeWalking. DB table `timewalking_levels` is empty.\n");
            LOG_INFO("server.loading", " ");
            return;
        }
        ...
        do
        {
            ...
            _pctMap[TIMEWALKING_AURA_MOD_STR_PCT] = timeWalkingLevel_Field[3].Get<uint32>();
            _pctMap[TIMEWALKING_AURA_MOD_AGI_PCT] = timeWalkingLevel_Field[4].Get<uint32>();
        ...
        {
            LOG_ERROR("Module", ">> Loaded 0 achievements for TimeWalking. DB table `azth_achievements` is empty.\n");
            LOG_INFO("server.loading", " ");
            return;
        }
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (!sAZTH->GetAZTHPlayer(player)->isTimeWalking()) {
            ...
            //  AddGossipItemFor(player,GOSSIP_ICON_TABARD, "All phases", GOSSIP_SENDER_MAIN, 5);
            //    AddGossipItemFor(player,GOSSIP_ICON_INTERACT_1, "Specific level", GOSSIP_SENDER_MAIN, 6, "Set a level", 0, true);
                if (player->IsGameMaster()) {
                    AddGossipItemFor(player,GOSSIP_ICON_TRAINER, "Scaling automatic (beta)", GOSSIP_SENDER_MAIN, TIMEWALKING_LVL_AUTO+10000);
                }
            ...
            AddGossipItemFor(player,0, "Exit TimeWalking mode", GOSSIP_SENDER_MAIN, 7);
        }
...
public:
    ...
    bool OnBeforeQuestComplete(Player *player, uint32 quest_id) override
    {
        ...
        CharacterDatabase.Execute("INSERT INTO quest_log (guid, groupId, quest, dungeon, difficulty, sLevel, nLevel, gSize, instanceStartTime, questEndTime) VALUES({},{},{},{},{},{},{},{},{},{});",
    ...
    void OnLoadFromDB(Player *player) override
    {
        ...
        if (!timewalkingCharactersActive_table)
        {
            LOG_WARN("server.loading", ">> Loaded 0 timewalking_characters_active. DB table `timewalking_characters_active` is empty.");
            LOG_INFO("server.loading", " ");
            return;
        }
        if (timewalkingCharactersActive_table) //if is in timewalking mode apply debuff
        {
            ...
            sAZTH->GetAZTHPlayer(player)->SetTimeWalkingLevel(timewalkingCharactersActive_field[1].Get<uint32>(), false, false , true);
        }
    }
    ...
    void OnLogin(Player* player) override
    {
        ...
        if (rep > 0)
        {
            QueryResult last_activity_rep = CharacterDatabase.Query("SELECT c.logout_time,MAX(questEndTime),MAX(instanceEndTime),MAX(instanceStartTime) FROM quest_log AS q LEFT JOIN characters AS c ON q.guid=c.guid WHERE q.guid = {};", player->GetGUID().GetCounter());
            if (last_activity_rep)
            {
                Field* last_activity_fields = last_activity_rep->Fetch();

                uint32 logoutTime=last_activity_fields[0].Get<uint32>();
                uint32 lastQuestEnd=last_activity_fields[1].Get<uint32>();
                uint32 lastInstanceEnd=last_activity_fields[2].Get<uint32>();
                uint32 lastInstanceStart=last_activity_fields[3].Get<uint32>();
    ...
    void OnAfterUpdateEncounterState(Map* map, EncounterCreditType /*type*/,  uint32 /*creditEntry*/, Unit* /*source*/, Difficulty difficulty_fixed, DungeonEncounterList const* encounters, uint32 dungeonCompleted, bool updated) override
    {
        ...
        for (size_t i = 0; i < list.size(); i++)
        {
            ...
            if (count >= total)
            {
                ...
                CharacterDatabase.Execute("INSERT INTO quest_log (guid, groupId, quest, dungeon, difficulty, sLevel, nLevel, gSize, instanceStartTime, instanceEndTime)"
                    "VALUES ({},{},{},{},{},{},{},{},{},{})", guid, id, 0, dungeon, difficulty, sLevel,nLevel,gSize,instanceStart,now);
            }
        }
    }
