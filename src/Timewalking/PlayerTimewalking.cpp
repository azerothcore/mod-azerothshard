...
void AzthPlayer::SetTimeWalkingLevel(uint32 itsTimeWalkingLevel, bool clearAuras/*=true*/, bool save /*=true*/, bool login/*=false*/)
{
    ...
    if (itsTimeWalkingLevel > 0)
    {
        ...
        if (save)
            QueryResult timewalkingCharactersActive_table = CharacterDatabase.Query(("INSERT IGNORE INTO timewalking_characters_active (`id`, `level`) VALUES ('{}', '{}');"), player->GetGUID().GetCounter(), sAZTH->GetAZTHPlayer(player)->GetTimeWalkingLevel());
    ...
    else
    {
        ...
        if (save) {
            QueryResult timewalkingCharactersActive_table = CharacterDatabase.Query(("DELETE FROM timewalking_characters_active WHERE  `id`= {};"), player->GetGUID().GetCounter());
